...
from nuvolos_cli.interface import nv_cli_config, res_teardown, res_remote_add, res_remote_remove
...
def class_proj(request):
    ...
    os.chdir(t)
    runner = CliRunner()
    verify_result(runner.invoke(nv_cli_config, ["--no-remote-setup"]))
    request.cls.t = t
    yield
    try:
